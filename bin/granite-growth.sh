...
PROJECT=$(bash ~/.hermes/scripts/pipeline-helper.sh growth 2>/dev/null | awk -F: '/^PROJECT:/ {gsub(/^ +| +$/,"",$2); print $2; exit}')
...
RESPONSE=$(echo "$PROMPT" | ~/.claude/bin/granite-bridge.sh --model qwen-coder --max-tokens 1400 2>>"$LOG")
[[ -z "$RESPONSE" ]] && exit 1
